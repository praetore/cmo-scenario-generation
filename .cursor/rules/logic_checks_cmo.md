...
- **Formation (mandatory)**: **Full CSG** in **`form_csg_group`** (lead = CVN): CVN + DDGs + **CG** (all escorts in one surface group). After `CreateMissionFlightPlan`, `setup_csg_strike_on_air_strike(CSG_GROUP, {cvn, ddg, cg, …})` unifies naval strike assets on the package mission while ships stay in formation — see `skills_cmo.md` §9.
