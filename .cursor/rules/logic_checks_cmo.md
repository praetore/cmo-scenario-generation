...
- **Magazines & Stores**: Units without ammunition in their magazines cannot rearm.
  ...
  - *Check (Scenario preflight — mandatory)*: Always run `python scripts/validate_scenario.py generated/<scenario>.lua --series <DB3K|CWDB> --version <version>` before delivering a script (scenarios live in `generated/`). Also checks **wrapper colon syntax** (`mission:updateWPtimes()`, not `mission.updateWPtimes()`). Validation runs against the matching source `.db3` (path via `cmo_config.ini` → game `DB/`, or local repo `DB/`).
  ...
  - *Check (Strike package escort — mandatory)*: **Non-stealth bombers** on a **Strike** mission and every **SEAD** mission must be supported by **fighter escort** (CAP/AAW with A/A loadout on the same side). SEAD flights also need their own escort (not only the strike wave).
    ...
    - **Heuristic in `scripts/preflight_checks_strike.py`** (re-exported via `preflight_checks.py`): classification on `DataAircraft.Name` (e.g. B-52, B-1, Tu-95, Tu-22, Su-24, F-111, …); stealth markers excluded. Missing escort fails `--validate-scenario`. Pass explicit Lua-side as the first argument of `add_air_unit_checked` / `spawn_air_wing`, otherwise the check cannot be enforced per side.
  ...
  - *Check (Strike timing — mandatory)*: **Hardcode one clock** — `local strike_package_tot = 'HH:MM:SS'` at scenario top; reuse for air `CreateMissionFlightPlan` `TIMEONTARGET`, naval `TimeOnTargetStation`, and `@strike_package` / `@naval_package` annotations. Derive `strike_tot_dt = mission_schedule_datetime(strike_package_date, strike_package_tot)` once **after OOB**; all timed `ScenEdit_SetMission` / flight plans **after** last `place_ship` / `spawn_air_wing` / `AssignUnitAsTarget`. TLAM: `setup_csg_strike_on_air_strike(group, {cvn, ddg, cg, …})` after the air flight plan (hardcoded launch + TOT via `set_naval_strike_schedule`). Preflight: **`Strike timing`** + **`Strike schedule order`**.
  ...
  - *Check (SEAD mission — mandatory)*: In CMO, **SEAD is always `ScenEdit_AddMission(..., 'Patrol', {type='SEAD', zone={...}})`**, never `Strike`. SEAD patrols engage **radar/emitter contacts within the zone**; SAMs on the strike target list are not automatically suppressed there.
    ...
    - **ISR/AEW on-orbit at scenario start (demo / short-talk scenarios)**: When `on_station` equals `scenario_start_time` and the player must see coverage **immediately at Play**, mission schedule alone is **not** enough for hosted aircraft. **Mandatory pattern:**
      - **First flight** (`Triton #1`, `Hawkeye #1`): `ScenEdit_AddUnit` with `type='Air'`, orbit `latitude`/`longitude`/`altitude`, **no** `base`, `mission=` set, then `assign_air_to_mission` + track in `spawned_air_missions`.
      - **Relief pool** (`#2…#N`): `add_air_unit_checked` on land base or carrier; same Support mission; `OnDeactivateUassign=false`.
      - **Support mission:** `FlightSize=1`, `MinAircraftReq=1` (not 2 — one on station, pool rotates).
      - **Init order:** run `set_patrol_on_station_schedule` for ISR/AEW, then **re-apply** orbit `SetUnit` after all strike/restore schedule steps.
      - **Play events:** `ScenLoaded` + `Time` at `scenario_start + 5s` with `ScenEdit_SetUnit({ launch=true, throttle='Cruise', … })` on on-orbit GUIDs.
      - **Do not** rely on `SetUnit` lat/lon on units spawned with `base=` — they remain on deck. Reference: `cuba_pressure_2026_demo_src.lua`.
    - **Play restore times when H-hour ≠ 00:00:00**: `add_mission_schedule_restore_event({ restore_times = { … } })` passes each `HH:MM:SS` to `mission_schedule_datetime(strike_package_date, hhmmss)` — **absolute clock**, not offset from scenario start. With `scenario_start_time = '03:00:00'`, use `'03:00:05'` (not `'00:00:05'`) as the first refresh trigger. Same for `add_strike_assign_restore_event`.
    ...
    - **Preflight**: `scripts/validate_scenario.py` also checks: `ScenEdit_SetTime` before SEAD takeoff; `@sead_package minutes_before_strike_tot` vs computed delta; SEAD not too early before TOT (before strike-escort launch); `@strike_package date` vs `DATEONTARGET`.
...
- **Formation (mandatory)**: **Full CSG** in **`form_csg_group`** (lead = CVN): CVN + DDGs + **CG** (all escorts in one surface group). After `CreateMissionFlightPlan`, `setup_csg_strike_on_air_strike(CSG_GROUP, {cvn, ddg, cg, …})` unifies naval strike assets on the package mission while ships stay in formation — see `AGENTS.md` §9.
...
- *Preflight*: `scripts/validate_scenario.py` (heuristic on `place_ship` + DB `DataShip` name/type).
...
- **Check**: `python scripts/validate_scenario.py generated/<file>.lua` reports **`Sides:`** errors for missing or out-of-order `ScenEdit_AddSide`. Resolves `local SIDE_X = 'France'` aliases used in AddSide/posture calls.
