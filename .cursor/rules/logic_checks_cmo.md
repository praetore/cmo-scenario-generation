...
- **Magazines & Stores**: Units without ammunition in their magazines cannot rearm.
  ...
  - *Check (Naval TLAM timing — mandatory)*: **Ships on the same Strike as air `CreateMissionFlightPlan` launch Tomahawks at scenario start** — put the CG on a **separate** Strike mission with hardcoded `starttime` + `TimeOnTargetStation` (~20–35 min before TOT), not on the air-strike mission. Document `-- @naval_package mission=<NavalStrikeName> launch=HH:MM:SS tot=HH:MM:SS minutes_before_strike_tot=N`. **CMO quirk:** CG in a **surface group** + TLAM assign often clears launch/TOT in ME — **preferred:** `setup_solo_tlam_shooter(cg)` (`group=''`, no naval flight plan) + `add_tlam_shooter_event(cg, '', …)`. **Init log:** `NOTE:`/`OK:` when `GetMission` empty at import — see `skills_cmo.md` §6; never `ERROR:` for that alone. Preflight: naval timing + **TLAM schedule workflow**.
  - *Check (TLAM shooter gun policy — mandatory when `@naval_package` / solo CG)*: TLAM shooter uses **Tomahawk only** for the land strike — **never** main gun on land (`M.WRA_GUN_LAND_BLOCK` / land target WRA `none`). **Surface gun:** allowed **only in self-defence** when threatened — unit `weapon_control_status_surface=0` (HOLD), `engage_opportunity_targets=false`, surface gun WRA `{ inherit, inherit, none, max }` (offensive firing range `none`, self-defence `max`). **No active hunting** of surface contacts. Mission `weapon_state_planned='ShotgunBVR'` (standoff weapons; no ToO/guns modes). Applied via `configure_tlam_shooter_weapon_policy()` in `setup_solo_tlam_shooter` + TLAM restore event. Preflight: **`TLAM shooter guns`** checks in `--validate-scenario`.
  ...
  - *Check (Mission assignment — mandatory)*: Every `place_ship` / `place_sub` / `place_sam` must get `ScenEdit_AssignUnitToMission` (directly or `for _, u in pairs(table)`). **Aircraft**: `spawn_air_wing` → existing mission; set `mission=` on `ScenEdit_AddUnit` (not for strike escort) **and** `assign_air_to_mission` (guid/name × mission name/guid, `SetUnit` fallback). **Never** `ScenEdit_SetMission` on air Strike after aircraft are assigned (clears ORBAT). Configure strike flight size via `configure_strike_mission_options` before spawn; air TOT via `CreateMissionFlightPlan` TIMEONTARGET only. After flight plan: `finalize_strike_air_after_flight_plan()` (restore **all** spawned air). If TLAM/naval init runs **after** that (e.g. `setup_solo_tlam_shooter` → naval `CreateMissionFlightPlan`), call `restore_all_spawned_air_assignments()` again at init end + `verify_spawned_air_assignments()`. Strike escort: `AssignUnitToMission(..., true)`. Preflight: **`Mission assignment`** + **`Aircraft mission assignment`**.
...
- **Formation (mandatory)**: CVN + **DDGs** in **`form_csg_group`** (lead = CVN). **CG TLAM shooter solo** (not in CSG group) → `setup_solo_tlam_shooter(cg)` after air flight plan; optional `add_tlam_shooter_event(cg, '', …)`. Legacy: `finalize_detached_tlam_shooter` / grouped `finalize_csg_tlam` — see `skills_cmo.md` §9.
- **TOT sync (mandatory)**: `local strike_package_tot` is the single impact time for **air** (`CreateMissionFlightPlan` TIMEONTARGET `YYYY/MM/DD`) and **TLAM** (`TimeOnTargetStation` via `sync_naval_strike_tot` / `mission_schedule_datetime` → `YYYY.MM.DD`). **Do not** call `CreateMissionFlightPlan` on the ship-only TLAM strike (clears ME TOT). Run `setup_solo_tlam_shooter` / `apply_naval_strike_flight_plan` **after** air flight plan. `@strike_package time=` and `@naval_package tot=` must match. Preflight: **Strike TOT sync** + **TLAM schedule workflow**.
...
- **Cruise missiles (TLAM)**: CG/DDG carry Tomahawk in VLS. Opening salvo: CG on a **separate timed Strike** (`<NavalStrikeName>`), **not** the carrier air-strike mission — see §1 *Naval TLAM timing* and **Formation** above. Preflight warns if CSG warships lack strike assignment or share the air-strike mission with aircraft.
- **F-35 variants**:
