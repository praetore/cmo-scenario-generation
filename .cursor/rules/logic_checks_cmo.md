...
- **Magazines & Stores**: Units without ammunition in their magazines cannot rearm.
  ...
  - *Check (Strike striker/escort counts — mandatory)*: **Tellen en documenteren** — houd per Strike-missie de **exacte aantallen** bij en controleer dat ze overeenkomen met de flight-size-instellingen. CMO de-assigneert strikers (vaak van een tweede basis, bv. Sigonella) als er te weinig escorts zijn voor het aantal strike flights.
    - **Strikers tellen:** som van alle `spawn_air_wing(..., '<StrikeMission>', base, …)` **zonder** `escort=true` (carrier én landbases).
    - **Escorts tellen:** som van alle `spawn_air_wing(..., '<StrikeMission>', base, true)` op **dezelfde** Strike-missie.
    - **Mission-opties:** `StrikeFlightSize`, `StrikeMinAircraftReq`, `EscortFlightSizeShooter`, `EscortMinShooter` via `configure_strike_mission_options` vóór spawn; documenteer in OOB-header en `-- @strike_package` (`flight_size`, `min_aircraft`, `escort_flight_size`, `min_ready_escort`).
    - **Vereiste escorts** (wanneer `StrikeUseFlightSize=true`):
      - `strike_flights = ceil(strikers ÷ StrikeFlightSize)`
      - `required_escorts = strike_flights × EscortMinShooter`
      - **Escorts geplaatst ≥ required_escorts** — anders faalt preflight en CMO laat deel van het pakket los.
    - **CMO trigger-minimum (kritiek):** bij `StrikeUseFlightSize=true` vermenigvuldigt de engine **`StrikeMinAircraftReq × StrikeFlightSize`** tot het minimum om de missie te starten (niet het ruwe MinReq). Voorbeeld: MinReq=8 + flight size 4 → **32** nodig, terwijl OOB 12 heeft → *"mission will never take off"*. Zet MinReq op **aantal strike flights** (bv. 2 voor 8 strikers in flights van 4), niet op totaal strikers. Escorts: vergelijkbaar `EscortMinShooter × EscortFlightSizeShooter`.
    - **Voorbeeld (Harmattan):** 8 strikers, `StrikeFlightSize=4` → 2 strike flights → `EscortMinShooter=2` → **minimaal 4 escorts**. Met slechts 2 escorts de-assigneert CMO de tweede flight (6× Rafale Sigonella).
    - **Alternatief (historische OOB met weinig escorts):** één wave — `StrikeFlightSize = strikers` (bv. 8) en `StrikeMinAircraftReq = strikers`; dan volstaan 2 escorts voor het hele pakket (minder realistisch voor CMO flight-size, wel dichter bij echte orde van grootte).
    - **CAP/SEAD op aparte missies** tellen **niet** mee als strike-escort (alleen `escort=true` op de Strike-missie).
    - **OOB vs mission (verplicht):** `spawn_air_wing`-aantallen moeten passen bij `configure_strike_mission_options` — **exacte telling**: `(strikers + escorts) ≥ StrikeMinAircraftReq × StrikeFlightSize`; escorts ≥ `ceil(strikers ÷ StrikeFlightSize) × EscortMinShooter`.
    - **Preflight:** `--validate-scenario` → **`Strike OOB vs mission`** (`_validate_strike_escort_coverage`); telt alle `spawn_air_wing`-rijen tegen mission-opties en `@strike_package`, niet alleen carrier.
